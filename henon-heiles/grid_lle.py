...
def compute_lle_row(i, y_val, py_vals, energy, t_total):
    start = time.perf_counter()

    ds = HamiltonianSystem(model="henon heiles")

    n_py = py_vals.size
    lle_row = np.full(n_py, np.nan, dtype=float)
    ...
    return i, lle_row, info_row, elapsed


def run_for_energy(energy, y, py, grid_size, t_total, max_workers, path_to_data):
    lle = np.full((grid_size, grid_size), np.nan, dtype=float)
    info_grid = np.full((grid_size, grid_size), -999, dtype=int)
    times_y = np.full(grid_size, np.nan, dtype=float)

    datafile = f"{path_to_data}/grid_lle_E={energy:.5f}_T={t_total:.0f}_grid_size={grid_size}.dat"
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(compute_lle_row, i, y_val, py, energy, t_total): i
            for i, y_val in enumerate(y)
        }

        for future in as_completed(futures):
            i, lle_row, info_row, elapsed = future.result()
